...
*/
    ...
    const Instruction* str2inst(const Function *func, string targetname){
        ...
        for(auto it = inst_begin(func); it != inst_end(func); ++it){
            ...
            if(inst->hasName()){
                ...
                if(n == targetname){
                    if(targetinst){
                        cout << "[Error]: Both instructions has the same name, " << targetname << endl;
   ...
   inline string inst2str(const Value *val){
    ...
    int getLineNum( Instruction *inst ) {
    ...
    int getLineNum( Value *val ) {
        if ( !( 27 <= val->getValueID() && val->getValueID() <= 94 ) ) {
            return 0;
        }
    ...
    bool isInst( Value *val ) {
        if ( 27 <= val->getValueID() && val->getValueID() <= 94 )
            return true;
        return false;
    }

    bool isTermOp( const Instruction *inst ) {
        unsigned int opcode = inst->getOpcode();
        if ( Instruction::TermOpsBegin <= opcode && opcode < Instruction::TermOpsEnd ) {
            return true;
        }
        return false;
    }

    bool isUnaryOp( const Instruction *inst ) {
        unsigned int opcode = inst->getOpcode();
        if ( Instruction::UnaryOpsBegin <= opcode && opcode < Instruction::UnaryOpsEnd ) {
            return true;
        }
        return false;
    }

    bool isBinaryOp( const Instruction *inst ) {
        unsigned int opcode = inst->getOpcode();
        if ( Instruction::BinaryOpsBegin <= opcode && opcode < Instruction::BinaryOpsEnd ) {
            return true;
        }
        return false;
    }

    bool isMemoryOp( const Instruction *inst ) {
        unsigned int opcode = inst->getOpcode();
        if ( Instruction::MemoryOpsBegin <= opcode && opcode < Instruction::MemoryOpsEnd ) {
            return true;
        }
        return false;
    }

    bool isCastOp( const Instruction *inst ) {
        unsigned int opcode = inst->getOpcode();
        if ( Instruction::CastOpsBegin <= opcode && opcode < Instruction::CastOpsEnd ) {
            return true;
        }
        return false;
    }

    bool isFuncletPadOp( const Instruction *inst ) {
        unsigned int opcode = inst->getOpcode();
        if ( Instruction::FuncletPadOpsBegin <= opcode && opcode < Instruction::FuncletPadOpsEnd ) {
            return true;
        }
        return false;
    }

    bool isOtherOp( const Instruction *inst ) {
        unsigned int opcode = inst->getOpcode();
        if ( Instruction::OtherOpsBegin <= opcode && opcode < Instruction::OtherOpsEnd ) {
            return true;
        }
        return false;
    }
    ...
    std::string getOperatorTypeName( const Instruction *inst ) {
        ...
        else if ( isUnaryOp( inst ) ) {
            result = "UnaryOp";
        }
        else if ( isBinaryOp( inst ) ) {
            result = "BinaryOp";
        }
        else if ( isMemoryOp( inst ) ) {
            result = "MemoryOp";
        }
        else if ( isCastOp( inst ) ) {
            result = "CastOp";
        }
        else if ( isFuncletPadOp( inst ) ) {
            result = "FuncletPadOp";
        }
        else if ( isOtherOp( inst ) ) {
            result = "OtherOp";
        }
        else {
            result = "Not Found";
